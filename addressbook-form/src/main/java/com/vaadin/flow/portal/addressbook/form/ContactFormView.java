...
public class ContactFormView extends VerticalLayout implements PortletView {
    ...
    public void onPortletViewContextInit(PortletViewContext context) {
        ...
        windowState = new Button(WindowState.NORMAL.equals(context.getWindowState()) ? STATE_MAXIMIZE : STATE_NORMALIZE, event -> switchWindowState());
        windowState.setId(WINDOW_STATE_CHANGE);

        portletMode = new Button(PortletMode.EDIT.equals(context.getPortletMode()) ? MODE_VIEW : MODE_EDIT, event -> switchPortletMode());
    ...
    private void switchWindowState() {
        if (WindowState.NORMAL.equals(context.getWindowState())) {
            ...
            Notification.show("Switching to state " + WindowState.MAXIMIZED);
            ...
            Notification.show("Switching to state " + WindowState.NORMAL);
        }
    }
    ...
    private void switchPortletMode() {
        if (PortletMode.EDIT.equals(context.getPortletMode())) {
            ...
            Notification.show("Switching to mode " + PortletMode.VIEW);
        } else {
            context.setPortletMode(PortletMode.EDIT);
            portletMode.setText(MODE_VIEW);
            Notification.show("Switching to mode " + PortletMode.EDIT);
        }
    }
